...
function handleKeyDown(event) {
    if (!isWalking && (event.key === 'ArrowLeft' || event.key === 'ArrowUp' || event.key === 'ArrowRight' || event.key === 'ArrowDown')) {
...
function handleKeyUp(event) {
    if (isWalking && (event.key === 'ArrowLeft' || event.key === 'ArrowUp' || event.key === 'ArrowRight' || event.key === 'ArrowDown')) {
        ...
        isWalking = false;
        document.querySelector('.teacher').style.backgroundImage = "url('/images/teacher.png')"; // Switch to default character image
    }
}
...
function startWalkingAnimation(direction) {
    ...
    intervalId = setInterval(function() {
        if (count % 2 === 0) {
            teacher.style.backgroundImage = "url('/images/teacher-walk.png')";
        } else {
            teacher.style.backgroundImage = "url('/images/teacher.png')";
        }
...
function moveTeacher(direction) {
    ...
    var teacherTop = parseInt(teacherStyle.top);
    var teacherLeft = parseInt(teacherStyle.left);
